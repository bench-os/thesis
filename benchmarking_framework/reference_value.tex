...
Every time a task is run and goes to the foreground, it set its GPIO up.
Once the task is finished and goes to the background, it reset its GPIO down.
...
The context switching time happen between the execution of the two tasks and is bordered with dotted lines in the schema.
...
The first task set up its GPIO on step A.
...
The oscilloscope measure the context switching time between the step B' and the step C'.
...
As described in the methodology, high value means that the task is running in the foreground and wow value means that the task is sleeping in the background.
